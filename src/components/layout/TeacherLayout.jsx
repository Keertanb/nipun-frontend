...
import { LayoutDashboard, Users, CheckCircle2, UserCircle, LogOut, Menu, GraduationCap } from 'lucide-react'
...
const items = [
  ...
  { to: '/teacher/completed', label: 'Completed Reviews', icon: CheckCircle2 },
    ...
    <div className="h-screen overflow-hidden flex gap-3 py-3 pr-3 bg-gradient-to-br from-sky-50 via-white to-leaf-50">
      ...
      <div className="relative flex-1 min-w-0 flex flex-col bg-white border border-sky-100 rounded-none lg:rounded-[2rem] shadow-soft overflow-hidden">
        ...
        <header className="relative shrink-0 overflow-hidden bg-gradient-to-r from-sky-50 to-white px-4 sm:px-6 py-4 flex items-center justify-between border-b border-sky-100">
          <div className="absolute -top-8 right-24 w-28 h-28 bg-leaf-200/40 rounded-full blur-2xl -z-10" />
          <StarDoodle className="w-5 h-5 top-2 right-40 hidden xl:block" color="#FFBE22" delay={0.4} />
          <ABCBlock className="w-7 h-7 -bottom-1 right-56 hidden xl:block" letter="T" color="#FA5411" delay={0.9} />
          <div className="flex items-center gap-3">
            ...
            <div className="hidden lg:flex items-center gap-3">
              ...
              <div>
                <p className="text-xs text-sky-700/60">Welcome back,</p>
                <p className="font-heading font-extrabold text-sky-900 leading-tight">{user?.teacher?.name || 'Teacher'}</p>
              </div>
            </div>
          </div>
          <div className="relative flex items-center gap-3">
            <Balloon className="w-6 h-11 top-1 right-16 hidden md:block" color="#22B566" delay={0.7} />
